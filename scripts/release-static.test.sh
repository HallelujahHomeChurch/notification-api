...
worker_runtime="$(sed -n '/^resource worker /,/^resource migrate /p' "${bicep}")"
...
grep -Fq 'enabled: true' <<<"${worker_runtime}" || fail "worker Dapr sidecar must be enabled"
...
assert_contains "${workflow}" 'properties\.configuration\.dapr' "worker Dapr release verification is missing"
